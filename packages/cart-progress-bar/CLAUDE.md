...
**Known regression vs. the old Rollup build: class names are NOT preserved in `dist/cart-progress-bar.min.js`.** Rolldown rewrites `class CartProgressBar extends HTMLElement {}` into `var CartProgressBar = class extends HTMLElement {}` before Terser runs, so `mangle.keep_classnames: true` has no named class to keep and the mangled binding becomes the inferred name — `CartProgressBar.name === 'r'` in the UMD. The ESM build is unaffected (`.name === 'CartProgressBar'`, via named evaluation of the unmangled `var`). This is pattern-wide across the `@magic-spells` Vite build script, not specific to this package. Only cosmetic today (devtools display, `constructor.name`); do not fix it here in isolation.
